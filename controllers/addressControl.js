...
const getAllAddress = asyncHandler(async (req, res) => {
  ...
  res.render("user/pages/address", { address, account: true, user ,dlt});
});
...
const editAddressPage = asyncHandler(async (req, res) => {
  ...
  // const address = await Address.find({ user_id: req.session.user_id });
  res.render("user/pages/editAddress", { newAddress, user });
});
...
const editAddress = async (req, res) => {
  try {
    ...
    res.render('user/pages/address', { address: addresses, user });
} catch (error) {
    console.error(error);
